    ...
    try:
        with connectionBD() as conexion_MySQLdb:
            with conexion_MySQLdb.cursor(dictionary=True) as cursor:
                ...
                sql = "INSERT INTO empleados (nombre_empleado, apelidos_empleado, tipo_identidad, n_identidad, fecha_nacimiento, sexo, grupo_rh, email, telefono, profesion, salario, foto_perfil) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
...
def procesar_imagen_perfil(foto):
    try:
        ...
        upload_dir = os.path.join(basepath, f'../static/img/foto_perf0il/')

        # Validar si existe la ruta completa de subida del archivo
        ...
        with connectionBD() as conexion_MySQLdb:
            with conexion_MySQLdb.cursor(dictionary=True) as cursor:
                querySQL = (f"""
                    ...
                    """)
                cursor.excute(querySQL,)
                empleadosBD = cursor.fetchall()
        return empleadosBD
    except Exception as e:
        print(
            f"Error en la funcion sql-lista_emleadosBD: {e}")
        return None
...
def empladosReporte():
    try:
        ...
        print(
            f"Error en la funcion empladosReporte: {e}")
        return None
    
# 6 FUNCION : Funcion que exporta un excel con Empleados
def generarReporteExcel():
    dataEmpleados = empladosReporte()
    ...
    # Agrgar los registros a la hoja
    for registro in dataEmpleados:
        ...
        fecha_regidtro = registro['fecha_regidtro']  

        # Agregar los valores de la hoja
        hoja.append((nombre_empleado, apellidos_empleado, tipo_identidad, n_identidad, fecha_nacimiento,
                     sexo, grupo_rh, email, telefono, profesion, salario, fecha_regidtro))
