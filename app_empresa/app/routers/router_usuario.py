...
def cpanelRecoveryPassUserBD():
    if request.method == 'POST' and 'usuario' in request.form and 'password' in request.form:
        usuario = request.form['usuario']
        email = request.form['email']
        password = request.form['password']

        resulData = recibeInsertRegisterUser(usuario, email, password)
        if (resulData !=0):
            flash('la cuenta fue creada corectamente.', 'success')
            return redirect(url_for('inicio'))
        else:
            return redirect(url_for('inicio'))
...
@app.route('/closed-sesion', methods=['GET'])
def cerraSesion():
    if request.method == 'GET':
        if 'conectado' in session:
            ...
            flash('tu sesion fue cerrada corectamente.', 'success')
            return redirect(url_for('inicio'))
        else:
            flash('recuerde debe inciar sesion.', 'error')
            return render_template(f'{PATH_URL_LOGIN}/base_login.html')